...
function contactSocialCardHtml() {
  const items = siteInfo.social
    .filter((s) => s.href)
    .map(
      (s) => `<li>
      <a class="contact-card__social-link" href="${s.href}" rel="noopener noreferrer" target="_blank" aria-label="${s.label}">
        <span class="contact-card__social-icon" aria-hidden="true">${SOCIAL_ICONS[s.label] ?? ''}</span>
      </a>
    </li>`,
    )
    .join('');
  if (!items) return '';
  return `<div class="contact-card__social">
    <p class="contact-card__social-title">Social</p>
    <ul class="contact-card__social-list" role="list">${items}</ul>
  </div>`;
}

function contactsMainHtml() {
  const addr = siteInfo.addressLines.join('<br />');
  const hoursDl = siteInfo.hours.map((h) => `<dt>${h.label}</dt><dd>${h.value}</dd>`).join('');
  const social = contactSocialCardHtml();
  return `
    <div class="contact-grid">
      <div class="contact-card">
        ...
        <p class="contact-card__actions">
          ...
          <span class="contact-card__sep" aria-hidden="true">·</span>
          <a href="${siteInfo.mapsHref}" rel="noopener noreferrer" target="_blank">Apri in Google Maps</a>
        </p>
        ${social}
      </div>
      <div class="contact-map">
        <iframe
          title="Mappa — Via Piazzano 66, Atessa"
    ...
    <section class="contact-hours-section" aria-labelledby="contatti-orari-title">
      <h2 id="contatti-orari-title" class="contact-hours-section__title">Orari</h2>
      <p class="contact-hours-section__intro">${siteInfo.hoursIntro}</p>
      <dl class="contact-hours-list">${hoursDl}</dl>
    </section>`;
}

/** Monta il contenuto principale della pagina contatti (dati da `siteInfo`). */
export function mountContactsPage() {
  const root = document.getElementById('contact-mount');
  if (root) root.innerHTML = contactsMainHtml();
}
...
export function mountChrome(active) {
  ...
  bottom.innerHTML = `
    <div class="bottom-nav__row">
      ...
      <a class="bottom-nav__whatsapp" href="${siteInfo.whatsappHref}" rel="noopener noreferrer" target="_blank" aria-label="Contattaci su WhatsApp">
